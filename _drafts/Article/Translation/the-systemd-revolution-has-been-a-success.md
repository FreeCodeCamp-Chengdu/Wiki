...
图1：2010年代的 systemd 之战异常激烈，伤亡惨重。

时间回到2013年，我当时气得跳脚。

`systemd` 正在用二进制格式取代我的纯文本日志，还给 `init` 打了激素，它还在嘲笑我。Unix 哲学在呐喊：这就是 Linux（或者，正如许多人称呼的，GNU 加 Linux）的终结吗？

时间来到2025年，我在这里忏悔。不仅 systemd 是传统 `init` 的合格继任者，我还认为它值得为其所做的事情辩护——尤其是在它最初（甚至现在莫名其妙地）遭遇如此敌意的情况下。没有软件是完美的——除了 [TempleOS][2]——但我认为 systemd 基本上是一个成功的故事，证明了许多可怕的预测（包括我自己的）都是错误的。我错了！
...
你有没有想过，仅仅"回收旧进程ID"对一个安全、健壮的 init 守护进程来说还**不够**？也许它还应该保护系统的其他部分，跟踪所需服务的存活状态？
